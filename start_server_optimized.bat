...
REM Iniciar BattlEye primeiro
echo (%time%) Iniciando BattlEye...
start "BattlEye" /min "battleye\BEServer_x64.exe"
timeout 5

REM Parametros otimizados para estabilidade
echo (%time%) Iniciando servidor DayZ...
start "DayZ Server" /high "DayZServer_x64.exe" -config=%serverConfig% -port=%serverPort% -profiles=%serverProfile% -cpuCount=%serverCPU% -mod=%MOD_LIST% -doLogs -adminLog -netLog -freezeCheck -limitFPS=200 -maxMem=4096

REM Reinicio a cada 6 horas (21600 segundos) ao inves de 4
timeout 21600
echo (%time%) Reiniciando servidor...
taskkill /im DayZServer_x64.exe /F
taskkill /im BEServer_x64.exe /F
timeout 30
goto start
